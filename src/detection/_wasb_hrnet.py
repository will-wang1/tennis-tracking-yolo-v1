"""HRNet architecture, vendored from nttcom/WASB-SBDT (MIT licensed - see
https://github.com/nttcom/WASB-SBDT/blob/main/LICENSE.md), itself adapted
from Microsoft's HRNet-Image-Classification reference implementation (also
MIT). This is the backbone WASB-SBDT's pretrained tennis ball detector
(`weights/wasb_tennis_pretrained.pth.tar`) uses - see
`src/detection/wasb_ball_detector.py` for the detector built on top of it.

Copied near-verbatim (channel/stage wiring must match the pretrained
checkpoint's state dict exactly) except `_make_deconv_layers`, which read
its config via `cfg.MODEL.EXTRA` attribute access in the original - that
assumed an OmegaConf config object from WASB-SBDT's own hydra-based
pipeline. This project passes a plain nested dict instead, so it's rewritten
to bracket access; behavior is identical since NUM_DECONVS=0 for the tennis
config in use here means the method's only other job (building deconv
layers) is a no-op either way.
"""
...
class HRNet(nn.Module):
    def __init__(self, cfg):
        ...
        stem_strides = cfg["MODEL"]["EXTRA"]["STEM"]["STRIDES"]
        stem_inplanes = cfg["MODEL"]["EXTRA"]["STEM"]["INPLANES"]
    ...
    def _make_final_layers(self, cfg, channels):
        ...
        return nn.ModuleList(
            [nn.Conv2d(in_channels=channels[scale], out_channels=self._frames_out, kernel_size=kernel_size) for scale in self._out_scales]
        )

    def _make_deconv_layers(self, input_channels):
        deconv_cfg = self.deconv_config
        deconv_layers = []
